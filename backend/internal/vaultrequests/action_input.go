...
type GenerateInput struct {
	...
	UsageNotes        []UsageNoteInput `json:"usage_notes,omitempty"`
	SharedProjectIDs  []int64          `json:"shared_project_ids,omitempty"`
}

type UsageNoteInput struct {
	Location string `json:"location"`
	Notes    string `json:"notes,omitempty"`
}

type SessionApplyInput struct {
	TargetRef string                  `json:"target_ref"`
	Items     []SessionSelectionInput `json:"items"`
}

type SessionSelectionInput struct {
	ItemID          int64 `json:"item_id"`
	SourceProjectID int64 `json:"source_project_id"`
	ReplaceExisting bool  `json:"replace_existing,omitempty"`
}

func (input GenerateInput) ProjectUsageNotes() []projectvault.UsageNote {
...
func (input SessionApplyInput) SessionSelections() []projectvault.SessionSelection {
...
func DecodeGenerateInput(input map[string]any) (GenerateInput, error) {
	var value GenerateInput
	err := decodeMap(input, &value)
	return value, err
}

func DecodeSessionApplyInput(input map[string]any) (SessionApplyInput, error) {
	var value SessionApplyInput
	err := decodeMap(input, &value)
	return value, err
}

func DecodeApprovalContext(input map[string]any) (ApprovalContext, error) {
	var value ApprovalContext
	err := decodeMap(input, &value)
	return value, err
}

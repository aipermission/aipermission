...
func assertConnectorHistoryRoutes(t *testing.T, scenario historyRoutesScenario) {
	...
	encryptedOtherSecret, err := recordcrypto.EncryptJSON(testRuntimeVault(t, fixture.server, fixture.server.activeRuntime()), fixture.server.activeRuntime().Identity().WorkspaceID, recordcrypto.ConnectorCredentialProfile, secondPGProfile.ID, map[string]any{"password": "other-secret"})
	if err != nil {
...
func assertAuditHistoryRoutes(t *testing.T, scenario historyRoutesScenario) {
	...
	runtimeAuditPage := decodeRouteResponse[pageResponse[observability.Record]](t, runtimeAuditResponse.Body.Bytes())
	...
	connectorAuditPage := decodeRouteResponse[pageResponse[observability.Record]](t, connectorAuditResponse.Body.Bytes())

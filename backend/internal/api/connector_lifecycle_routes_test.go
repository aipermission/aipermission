...
func TestTargetsListHidesArchivedAndMismatchedProfiles(t *testing.T) {
	...
	ctx := context.Background()
	store := connectortargets.NewStore(fixture.db)
	secretVault := testRuntimeVault(t, fixture.server, fixture.server.activeRuntime())
	target, profile := createAPITestPostgresTargetProfile(t, store, secretVault, fixture.server.activeRuntime().Identity().WorkspaceID)

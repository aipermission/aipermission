...
func TestConnectorCredentialBoundaryAcrossRESTMCPHistoryAndAudit(t *testing.T) {
	...
	encryptedSecret, err := recordcrypto.EncryptJSON(testRuntimeVault(t, fixture.server, runtime), runtime.Identity().WorkspaceID, recordcrypto.ConnectorCredentialProfile, profile.ID, map[string]any{"password": credentialSecret})
	if err != nil {

...
func TestConnectorPeerTrustChangeInvalidatesVaultStateBeforeMutation(t *testing.T) {
	...
	if err := runtime.Security.VaultLeaseStore().Grant(lease); err != nil {
		t.Fatalf("grant Vault lease: %v", err)
	}
	if err := vaultsessions.NewPersistence(runtime.Storage.DatabaseHandle()).Grant(ctx, project.ID, lease); err != nil {
...
func TestIdenticalTokenAuthorizationUpdatesPreserveVaultSessionState(t *testing.T) {
	...
	if err := runtime.Security.VaultLeaseStore().Authorize(ctx, principal, authorization, console.OperationObserve); err != nil {

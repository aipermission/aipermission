...
func TestFileTransferControlRoutesDriveRegisteredBatch(t *testing.T) {
	...
	batch, err := filetransfer.NewStore(runtime.database).CreateBatch(t.Context(), filetransfer.CreateBatchRequest{
	...
	if changed, err := filetransfer.NewStore(runtime.database).MarkBatchRunning(t.Context(), batch.ID); err != nil || !changed {
	...
	request := func(action string, wantCode int, wantStatus string) {
		...
		stored, err := filetransfer.NewStore(runtime.database).GetBatch(t.Context(), batch.ID)
...
func TestFileTransferCancelSignalsWorkerOnlyAfterTerminalStateIsDurable(t *testing.T) {
	...
	runtime := fixture.server.activeRuntime()
	item, err := filetransfer.NewStore(runtime.database).Create(t.Context(), filetransfer.CreateRequest{
		RuntimeID: identity.TransferRuntimeID, Direction: filetransfer.DirectionUpload, RemotePath: "/report", FileName: "report",
	})
	if err != nil {
		t.Fatal(err)
	}
	if changed, err := filetransfer.NewStore(runtime.database).MarkRunning(t.Context(), item.ID); err != nil || !changed {
	...
	stored, err := filetransfer.NewStore(runtime.database).Get(t.Context(), item.ID)

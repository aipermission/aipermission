...
func TestHistoryRetentionSummarizesFileTransferDeletionWithoutPerRowAudit(t *testing.T) {
	...
	record, err := filetransfer.NewStore(runtime.StoragePort().DatabaseHandle()).Create(context.Background(), filetransfer.CreateRequest{
		RuntimeID:  server.ID,
	...
	if ok, err := filetransfer.NewStore(runtime.StoragePort().DatabaseHandle()).MarkRunning(context.Background(), record.ID); err != nil || !ok {
		t.Fatalf("mark running: ok=%v err=%v", ok, err)
	}
	if ok, err := filetransfer.NewStore(runtime.StoragePort().DatabaseHandle()).Complete(context.Background(), record.ID, 3, "checksum"); err != nil || !ok {
...
func TestFileTransferRoutes(t *testing.T) {
	...
	if ok, err := filetransfer.NewStore(runtime.StoragePort().DatabaseHandle()).Complete(context.Background(), record.ID, int64(len("download payload")), "abc123"); err != nil || !ok {
	...
	cancelRecord, err := filetransfer.NewStore(runtime.StoragePort().DatabaseHandle()).Create(context.Background(), filetransfer.CreateRequest{
		RuntimeID:  server.ID,
		Direction:  filetransfer.DirectionUpload,
		Source:     filetransfer.SourceUI,
		LocalPath:  "movie.mp4",
	...
	if ok, err := filetransfer.NewStore(runtime.StoragePort().DatabaseHandle()).MarkRunning(context.Background(), cancelRecord.ID); err != nil || !ok {
	...
	batch, err := filetransfer.NewStore(runtime.StoragePort().DatabaseHandle()).CreateBatch(context.Background(), filetransfer.CreateBatchRequest{
		RuntimeID: server.ID,
		Direction: filetransfer.DirectionUpload,
		Source:    filetransfer.SourceUI,
	...
	if ok, err := filetransfer.NewStore(runtime.StoragePort().DatabaseHandle()).MarkBatchRunning(context.Background(), batch.ID); err != nil || !ok {
		t.Fatalf("mark batch running: ok=%v err=%v", ok, err)
	}
	if ok, err := filetransfer.NewStore(runtime.StoragePort().DatabaseHandle()).PauseBatch(context.Background(), batch.ID); err != nil || !ok {

...
func TestConnectorPeerTrustChangeInvalidatesVaultStateBeforeMutation(t *testing.T) {
	...
	result, err := fixture.db.ExecContext(ctx, `
		...
		target.ID, token.ID, runtime.Identity().WorkspaceID, runtime.Identity().RuntimeID, now, now,
	)
	if err != nil {
	...
	if err := testRuntimeLeases(t, fixture.server, runtime).Grant(lease); err != nil {
		t.Fatalf("grant Vault lease: %v", err)
	}
	if err := vaultsessions.NewPersistence(fixture.db).Grant(ctx, project.ID, lease); err != nil {
	...
	changeCalled := false
	if err := fixture.server.connectorRuntime.MutationRouteGateway().ConnectorChangeVaultPeerTrust(ctx, func() error {
		changeCalled = true
...
func TestIdenticalTokenAuthorizationUpdatesPreserveVaultSessionState(t *testing.T) {
	...
	principal, err := executionprincipal.MCPToken(token.ID, runtime.Identity().WorkspaceID, runtime.Identity().RuntimeID)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := testRuntimeLeases(t, fixture.server, runtime).Authorize(ctx, principal, authorization, console.OperationObserve); err != nil {

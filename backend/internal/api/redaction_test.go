...
func TestCommandRequestKeepsEncryptedRawCommandForExecution(t *testing.T) {
	...
	id, err := runtime.Operations.CommandRequests.Insert(ctx, commandrequests.Insert{
	...
	record, err := runtime.Operations.CommandRequests.Get(ctx, id, token.ID, commandRequestSourceMCP)
	if err != nil {
	...
	executionCommand, err := runtime.Operations.CommandRequests.ExecutionCommand(ctx, id)
	if err != nil {
...
func TestCommandRequestErrorsAreRedactedBeforePersistence(t *testing.T) {
	...
	if err := runtime.Operations.CommandRequests.Finish(ctx, commandrequests.Completion{

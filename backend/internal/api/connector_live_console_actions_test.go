...
func TestLiveConsoleTargetRefPreservesUnexpectedAdapterErrors(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err := runtime.Connectors.ConnectorAdapterRegistry().Register(localActionTestConnectorKind, liveConsoleLookupTestAdapter{err: test.err}); err != nil {
				t.Fatal(err)
			}
...
func TestLiveConsoleTargetRefContinuesOnlyForRuntimeNotFound(t *testing.T) {
	...
	if err := runtime.Connectors.ConnectorAdapterRegistry().Register(localActionTestConnectorKind, liveConsoleLookupTestAdapter{err: connectortargets.ErrRuntimeSurfaceNotFound}); err != nil {
		t.Fatal(err)
	}
...
func TestLiveConsoleTargetRefRejectsEmptySuccessfulReference(t *testing.T) {
	...
	if err := runtime.Connectors.ConnectorAdapterRegistry().Register(localActionTestConnectorKind, liveConsoleLookupTestAdapter{}); err != nil {
		t.Fatal(err)
	}
...
func TestBulkConsoleTargetRejectsRuntimeWithoutCommandActionCapability(t *testing.T) {
	...
	if err := runtime.Connectors.ConnectorAdapterRegistry().Register(localActionTestConnectorKind, liveConsoleLookupTestAdapter{ref: targetRef}); err != nil {
		t.Fatal(err)
	}

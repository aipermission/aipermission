...
func assertS3TransferSizeLimits(t *testing.T, scenario s3TransferRouteScenario) {
	...
	pendingBatch, err := filetransfer.NewStore(fixture.db).CreateBatch(context.Background(), filetransfer.CreateBatchRequest{
	...
	rejectedBatch, err := filetransfer.NewStore(fixture.db).GetBatch(context.Background(), pendingBatch.ID)

...
func runMigrationMaintenance(database *sql.DB) error {
	...
	for _, statement := range []string{
		...
		`UPDATE file_transfers SET status = 'failed', error = 'gateway restarted while file transfer was running', failure_kind = 'interrupted', completed_at = COALESCE(completed_at, datetime('now')), updated_at = datetime('now') WHERE status IN ('pending', 'pending_approval')`,
		`UPDATE file_transfers SET status = 'failed', error = 'gateway restarted while file transfer was running', failure_kind = 'outcome_unknown', failure_details_json = CASE WHEN remote_staging_ref != '' THEN json_patch(failure_details_json, json_object('remote_cleanup_pending', json('true'), 'recovery_hint', 'Connector-owned staging cleanup will be retried when this workspace opens.')) ELSE failure_details_json END, completed_at = COALESCE(completed_at, datetime('now')), updated_at = datetime('now') WHERE status IN ('running', 'paused')`,
		`UPDATE file_transfer_batches SET status = 'failed', error = 'gateway restarted while file transfer queue was running', failure_kind = 'interrupted', completed_at = COALESCE(completed_at, datetime('now')), updated_at = datetime('now') WHERE status IN ('pending', 'pending_approval')`,
		`UPDATE file_transfer_batches SET status = 'failed', error = 'gateway restarted while file transfer queue was running', failure_kind = 'outcome_unknown', completed_at = COALESCE(completed_at, datetime('now')), updated_at = datetime('now') WHERE status IN ('running', 'paused')`,
		...
		`UPDATE history_entries SET status = 'failed', error = 'gateway restarted while file transfer was running', preview_json = json_object('failure_kind', 'interrupted'), completed_at = COALESCE(completed_at, datetime('now')), updated_at = datetime('now') WHERE source_ref_type = 'file_transfer' AND status IN ('pending', 'pending_approval')`,
		`UPDATE history_entries SET status = 'failed', error = 'gateway restarted while file transfer was running', preview_json = COALESCE((SELECT json_patch(json_object('failure_kind', 'outcome_unknown'), ft.failure_details_json) FROM file_transfers ft WHERE ft.id = history_entries.source_ref_id), json_object('failure_kind', 'outcome_unknown')), completed_at = COALESCE(completed_at, datetime('now')), updated_at = datetime('now') WHERE source_ref_type = 'file_transfer' AND status IN ('running', 'paused')`,
	} {
		if _, err := tx.Exec(statement); err != nil {

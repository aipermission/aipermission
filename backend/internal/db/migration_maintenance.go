...
func runMigrationMaintenance(database *sql.DB) error {
	...
	for _, statement := range []string{
		...
		`UPDATE file_transfers SET status = 'failed', error = 'gateway restarted while file transfer was running', failure_kind = 'interrupted', completed_at = COALESCE(completed_at, strftime('%Y-%m-%dT%H:%M:%f000000Z', 'now')), updated_at = strftime('%Y-%m-%dT%H:%M:%f000000Z', 'now') WHERE status IN ('pending', 'pending_approval')`,

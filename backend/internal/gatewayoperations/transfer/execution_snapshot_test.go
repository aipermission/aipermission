...
func TestTransferBrowseUsesOneAcceptedExecutionSnapshot(t *testing.T) {
	for _, testCase := range []struct {
		...
		handler func(*FileTransferHTTPHandlers, http.ResponseWriter, *http.Request)
	}{
		{
			name: "browse",
			...
			handler: func(handlers *FileTransferHTTPHandlers, response http.ResponseWriter, request *http.Request) {

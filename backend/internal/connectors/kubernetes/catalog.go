...
func (Connector) GetActionList(context.Context, connectors.TargetView, connectors.CredentialProfileView) ([]connectors.ActionDefinition, error) {
	return []connectors.ActionDefinition{
		...
		{Name: ActionListNamespaces, Label: "List namespaces", Description: "List visible namespaces; selected scopes return configured names without requiring cluster-wide namespace listing.", Category: "browser", Risk: connectors.RiskRead, InputSchema: connectors.Schema{}, OutputHint: connectors.OutputHint{Format: "json", MaxRows: 500}},

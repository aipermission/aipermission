...
The gateway is designed only for a localhost trust boundary. Docker Compose publishes host ports on `127.0.0.1` by default for portable IPv4 loopback behavior. The HTTP boundary consistently recognizes `localhost`, IPv4 loopback, and IPv6 loopback (`[::1]`) when an operator explicitly publishes a different loopback address. The backend rejects missing, malformed, or non-local remote-client addresses plus missing, malformed, or non-localhost Host headers. It also refuses to start when `AIPERMISSION_BACKEND_HOST` is `0.0.0.0` or any non-loopback address.

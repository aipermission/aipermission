...
The web REST API is not a remote multi-user API. After database unlock, protected web REST endpoints require a local HttpOnly browser session cookie. Mutating requests send a double-submit CSRF header/cookie pair and `X-AIPermission-Workspace`, the workspace identity observed by that browser tab. Recovery, diagnostics, and file download GETs also require the workspace header so a workspace switch cannot return data from a different unlocked database. A valid session without the required matching workspace header is rejected with `409 Conflict`; clients must refresh instead of replaying the operation against another workspace. Setup and recovery mutations that can run while locked require the header only after a workspace is unlocked. MCP endpoints do not use browser cookies or this workspace header; they authenticate with API tokens.
...
core or adapter route and schema drift.
Shared error, target/profile/action, approval, history, and audit response
families are typed. Operations that have not yet migrated remain explicitly
marked `route-inventory`; this document remains authoritative for those request
and response shapes.
...
UI. It does not expose local database file paths. The `audit` object combines
process-local write diagnostics with durable outbox health: pending event
count, oldest pending timestamp, retried event count, dispatcher failure count,
last delivery error/failure, and last successful delivery. A `degraded` status
means the local audit projection needs attention; committed outbox events remain
in the encrypted database for retry. Security-sensitive local mutations and
their audit intent commit atomically. Read observations and remote-side-effect
telemetry are a separate best-effort class. See
...
field matching the connector target name exactly and a caller-stable
`idempotency_key`. Before dispatch, the gateway hashes the complete artifact
and durably claims that key for the exact target, profile, filename, size, and
digest. Replays never dispatch a second restore, and a key reused for another
artifact is rejected. The file limit applies to the artifact itself; the HTTP
boundary allows only a small, bounded multipart envelope beyond it. The
built-in Postgres connector validates the exact declared size and rejects psql
...
files and appends a one-time
completion marker to prove that `psql` consumed the complete upload. SQL with
top-level explicit transaction control is reported as `outcome_unknown` even
when that marker is observed. `PREPARE TRANSACTION` is included in this
boundary; ordinary SQL `PREPARE name AS ...` remains valid. Every failure or
missing completion marker after process start is also terminal
`outcome_unknown`; inspect the target database before deciding whether to retry.
Every dispatched restore records a required terminal audit event, including
failed, canceled, and uncertain outcomes. An audit persistence failure is
reported explicitly and remains attached to the durable restore operation; it
must not be retried without inspecting the target.

The route holds the workspace's exclusive target/profile mutation gate from
profile resolution through terminal persistence. Configured history retention
later removes terminal operation detail but leaves a non-secret 30-day replay
tombstone. Reusing the same key returns its recorded terminal status; reusing it
for a different artifact remains a conflict.
...
functions, restricts read-query name resolution to `pg_catalog`, caps rows and
output bytes, and applies a statement timeout. Application relations therefore
must be schema-qualified, for example `public.orders`. Unknown,
extension-provided, schema-qualified non-catalog, and quoted function calls are
rejected. Operators should still use dedicated
...
Uploads are staged in a private, workspace-namespaced local temporary
directory. One unlocked database's scavenger cannot inspect or remove another
workspace's staging files. SSH transfers copy
...
The local staging file is removed after a completed or definite failed upload.
When the remote commit outcome is unknown, the gateway retains the bounded
staging artifact until its normal private-temp TTL expires and records the
...
Existing directories or virtual prefixes are rejected.
...
is rejected so the operator can choose another name explicitly. Normalized
database identifiers are at most 63 characters and reserve internal aliases
such as `local-default`. Older files created with an overlong or reserved
identifier remain available through a non-mutating recovery reference in the
database catalog and can then be renamed normally.

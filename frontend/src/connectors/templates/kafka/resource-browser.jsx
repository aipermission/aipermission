  ...
  return (
    ...
    >
      <div className={`grid grid-cols-2 gap-1 border-b p-2 ${styles.border}`} role="tablist" aria-label={`${browser.product} browser view`}>
        <ViewButton selected={browser.view === "topics"} onClick={() => void browser.changeView("topics")} icon={Database}>
          Topics
        </ViewButton>
        <ViewButton selected={browser.view === "groups"} onClick={() => void browser.changeView("groups")} icon={Users}>
          Groups
        </ViewButton>
      </div>
      <div className={`grid grid-cols-[minmax(0,1fr)_auto] gap-2 border-b p-3 ${styles.border}`}>
        <div className="relative">
          ...
          <Input
            ...
            placeholder={`Filter ${browser.view}`}
            aria-label={`Filter ${browser.view}`}
          />
        </div>
        <Button
          type="button"
      ...
      <div className="min-h-0 overflow-auto p-2">
        {browser.filteredItems.map((item) => (
          <button
            key={item.name}
            type="button"
            className={`mb-1 grid w-full gap-1 rounded-md border px-3 py-2 text-left transition ${browser.selectedName === item.name ? styles.activeRow : `${styles.border} ${styles.rowHover}`}`}
            onClick={() => void browser.selectItem(item)}
            aria-pressed={browser.selectedName === item.name}
          >
            <span className="truncate font-mono text-xs font-semibold" title={item.name}>
              {item.name}
            </span>
            <span className={`truncate text-xs ${browser.selectedName === item.name ? "" : styles.muted}`}>
              {itemSummary(browser.view, item)}
            </span>
          </button>
        ))}
        {browser.filteredItems.length === 0 ? (
          <Notice>{browser.state.state === "loading" ? `Loading ${browser.view}...` : `No ${browser.view} found.`}</Notice>
        ) : null}
      </div>
    </section>
  );

    ...
    <section className={`grid min-h-0 grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden rounded-lg border ${styles.border}`}>
      ...
      <div className="grid min-h-0 gap-4 overflow-y-auto p-4 lg:grid-cols-2 lg:overflow-hidden">
        <div className="grid min-h-0 grid-rows-[auto_minmax(0,1fr)] gap-2">
          <p className={`text-xs font-semibold uppercase ${styles.muted}`}>
            {browser.view === "topics" ? "Topic metadata" : "Group and lag"}
          </p>
          <TerminalBlock surface="log" className="min-h-0 text-xs">
            {browser.activeDetail ? JSON.stringify(browser.activeDetail, null, 2) : "No item selected."}
          </TerminalBlock>
        </div>
      ...
      <div className={`grid gap-2 border-t p-3 ${styles.border}`} aria-live="polite">
        ...
        <Notice tone="warn">
          Publishing writes one message. Offset changes can replay or skip messages. Local browser writes require confirmation here; for MCP
          access, keep both actions on Prompt unless direct execution is intentional.
        </Notice>
    ...
    <div className={`flex items-center justify-between gap-3 border-b p-3 ${styles.border} ${styles.subtlePanel}`}>
      <div className="min-w-0">
        <p className="truncate text-sm font-semibold">
          {browser.selectedName || (browser.view === "topics" ? "Topic detail" : "Consumer group detail")}
        </p>
        <p className={`truncate text-xs ${styles.muted}`}>
          {browser.selectedName
            ...
            : `Select one of the ${browser.view} on the left.`}
        </p>
      </div>
      <div className="flex items-center gap-2">
        {browser.activeDetail && browser.view === "topics" ? (
          <Button
            type="button"
            variant="outline"
        ...
        {browser.activeDetail && browser.view === "groups" && writes.offsetPartitions.length > 0 ? (
          <Button
            type="button"
            variant="outline"
            className="h-8 px-2 text-xs"
        ...
        {browser.activeDetail ? (
          <CopyButton
            value={JSON.stringify({ detail: browser.activeDetail, messages: browser.messages }, null, 2)}
            variant="outline"
            className="h-8 px-2 text-xs"
          >
            JSON
  ...
  return (
    <div className="grid gap-2 sm:grid-cols-[100px_140px_110px_minmax(0,1fr)_auto]">
      <Select
        ...
        value={browser.readForm.partition}
        onChange={(event) => update({ partition: event.target.value })}
        aria-label="Partition"
      >
        {(browser.activeDetail?.partitions || []).map((partition) => (
      ...
      <Select
        className={styles.input}
        value={browser.readForm.start_position}
        onChange={(event) => update({ start_position: event.target.value })}
      ...
      <Input
        ...
        type="number"
        min="0"
        value={browser.readForm[numberField]}
        onChange={(event) => update({ [numberField]: event.target.value })}
        aria-label={numberField === "offset" ? "Offset" : "Maximum records"}
      />
      <span />
      <Button
        type="button"

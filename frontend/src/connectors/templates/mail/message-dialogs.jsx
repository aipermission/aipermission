...
export function MoveMessageDialog({ dialog, folders, busy, onClose, onConfirm, onDestination }) {
  return (
    <Dialog
      open={dialog.open}
      title="Move message"
      description="Move this exact IMAP UID to an allowed existing folder."
      onClose={onClose}
      closeDisabled={busy}
      ...
      <div className="grid gap-4">
        <Field>
          ...
          <Select value={dialog.destination} onChange={(event) => onDestination(event.target.value)}>
            ...
            {folders
              .filter((folder) => !mailFolderEqual(folder.name, dialog.sourceFolder))
              .map((folder) => (
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="button" onClick={onConfirm} disabled={busy || !dialog.destination}>
            {busy ? "Moving..." : "Move message"}
          </Button>
        </div>
      </div>
    </Dialog>
...
export function DeleteMessageDialog({ open, trashFolder, busy, onClose, onConfirm }) {
  return (
    <Dialog
      ...
      description="This does not permanently expunge the message."
      onClose={onClose}
      closeDisabled={busy}
      closeOnOverlay={false}
      size="md"
    >
      <div className="grid gap-4">
        <Notice tone="warn">
          The selected message will move to {trashFolder || "the configured Trash folder"}. AIPermission never performs permanent IMAP
          expunge.
        </Notice>
        <div className="flex justify-end gap-2">
          <Button type="button" variant="outline" onClick={onClose} disabled={busy}>
...
export function RetryUnknownSubmissionDialog({ value, busy, onClose, onConfirm }) {
  return (
    <Dialog
      ...
      title="Retry an unknown SMTP submission?"
      description="The server may already have accepted the previous message."
      onClose={onClose}
      closeDisabled={busy}
      closeOnOverlay={false}
      size="md"
        ...
        <Notice tone="bad">
          ...
          {value?.draftChanged ? (
            <span className="mt-2 block">
              The draft changed after the unknown result, but the earlier message may still have been delivered.
            </span>
          ) : null}

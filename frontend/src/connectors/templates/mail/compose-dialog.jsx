...
export function ComposeDialog({ draft, busy, error, onClose, onSubmit }) {
  ...
  return (
    <Dialog
      ...
      title={draft?.reply ? "Reply" : "Compose message"}
      description="Review every recipient and the complete bounded body before submitting."
      onClose={onClose}
    ...
    >
      <form className="grid gap-4" onSubmit={submit}>
        <Notice tone="warn">
          SMTP acceptance does not guarantee delivery. An unknown submission result must not be retried automatically.
        </Notice>
        ...
        {mode === "plain" ? (
          <Field>
            ...
            <Textarea
              ...
              onChange={(event) => updateForm({ text_body: event.target.value })}
              required
            />
          </Field>
        ) : (
          <>
            ...
            <Field>
              ...
              <span className="text-xs font-normal text-stone-500">
                This complete fallback is included in the approval preview and multipart message.
              </span>
            </Field>
          </>
        )}
    ...
    <Field>
      ...
      <div className="overflow-hidden rounded-md border border-stone-300 bg-white">
        <div className="flex flex-wrap gap-1 border-b border-stone-200 p-1">
          ...
          <FormatButton title="Italic" onClick={() => command("italic")}>
            <Italic className="h-4 w-4" />
          </FormatButton>
          <FormatButton title="Underline" onClick={() => command("underline")}>
            <Underline className="h-4 w-4" />
          </FormatButton>
          <FormatButton title="Heading" onClick={() => command("formatBlock", "h2")}>
            <Heading2 className="h-4 w-4" />
          </FormatButton>
          <FormatButton title="Bulleted list" onClick={() => command("insertUnorderedList")}>
            <List className="h-4 w-4" />
          </FormatButton>
          <FormatButton title="Numbered list" onClick={() => command("insertOrderedList")}>
            <ListOrdered className="h-4 w-4" />
          </FormatButton>
          <FormatButton title="Quote" onClick={() => command("formatBlock", "blockquote")}>
            <Quote className="h-4 w-4" />
          </FormatButton>
          <FormatButton title="Code block" onClick={() => command("formatBlock", "pre")}>
            <Code2 className="h-4 w-4" />
          </FormatButton>
          <FormatButton title="Link" onClick={openLinkEditor}>
        ...
        {linkEditor.open ? (
          <div className="flex flex-wrap items-center gap-2 border-b border-stone-200 p-2">
            <Input
              ...
              onChange={(event) => setLinkEditor({ open: true, value: event.target.value, error: "" })}
              onKeyDown={(event) => {
            ...
            <Button
              ...
              variant="outline"
              className="h-8"
              onClick={() => setLinkEditor({ open: false, value: "https://", error: "" })}
            >
              Cancel
            </Button>
...
function AddressField({ label, value, onChange, required = false }) {
  return (
    <Field>
      {label}
      <Input value={value} onChange={(event) => onChange(event.target.value)} placeholder="name@example.com" required={required} />
    </Field>
  );
}

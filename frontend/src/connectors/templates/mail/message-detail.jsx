  ...
  if (!message) {
    return (
      <div className={`grid h-full place-items-center p-8 text-center text-sm ${mutedClass}`}>
        Select a message to read its bounded safe-text body.
      </div>
    );
  }
  return (
    <section className="grid h-full min-h-0 grid-rows-[auto_auto_minmax(0,1fr)] overflow-hidden">
      ...
      <div className="min-h-0 overflow-auto p-4">
        <pre className={`min-h-full whitespace-pre-wrap break-words rounded-md p-4 font-mono text-sm leading-6 ${subtlePanelClass}`}>
          {message.body_available === false ? "No safe text body is available." : message.body || "(empty message body)"}
        </pre>
        <AttachmentMetadata attachments={message.attachments} mutedClass={mutedClass} borderClass={borderClass} />
      </div>
    </section>
  );
}
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className={`mt-1 text-xs ${mutedClass}`}>
            {addressLabel(message.from)} · {formatMessageDate(message.received_at || message.header_date)}
          </p>
        </div>
        <MessageActions
          message={message}
...
function MessageActions({ message, busy, canReply, canMove, canArchive, canDelete, onToggleRead, onReply, onMove, onArchive, onDelete }) {
  return (
    <div className="flex flex-wrap items-center gap-1">
      ...
      <ActionIcon title={canReply ? "Reply" : "Reply unavailable: SMTP is disabled"} onClick={onReply} disabled={busy || !canReply}>
        <Reply className="h-4 w-4" />
      </ActionIcon>
      <ActionIcon
        title={canMove ? "Move" : "Move unavailable: no destination folder is allowed"}
        onClick={onMove}
      ...
      <ActionIcon
        title={canArchive ? "Archive" : "Archive unavailable: no Archive folder is configured"}
        onClick={onArchive}
      ...
      <ActionIcon
        title={canDelete ? "Move to Trash" : "Delete unavailable: no Trash folder is configured"}
        onClick={onDelete}
  ...
  return (
    <div className="mt-4 grid gap-2">
      ...
      {attachments.map((attachment, index) => (
        <div key={`${attachment.filename || "attachment"}:${index}`} className={`rounded border p-2 text-xs ${borderClass}`}>
          {attachment.filename || "unnamed"} · {attachment.content_type || "unknown type"} · {attachment.declared_size_bytes || 0} bytes
        </div>
      ))}
    </div>
  );

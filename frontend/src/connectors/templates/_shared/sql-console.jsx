...
function SQLNoSessionPlaceholder({ config, target, theme, onNewSession }) {
...
function SQLEndpointFooter({ config, target, borderClass, mutedClass }) {
  return (
    <div className={`border-t px-4 py-2 text-xs ${borderClass} ${mutedClass}`}>
      <span className="inline-flex min-w-0 items-center gap-2">
        <Database className="h-3.5 w-3.5 shrink-0" />
...
function SQLEditor({ value, onChange, onSubmit, focusSignal, theme, tables, keywords, disabled }) {
  ...
  useEffect(() => {
    ...
    loadMonaco().then((monacoInstance) => {
      ...
      const editor = monacoInstance.editor.create(containerRef.current, {
        value: value || "",
        language: "sql",
        theme: sqlEditorTheme(monacoInstance, theme),
...
function SQLSchemaBrowser({ rows, search, onSearch, onPrepareQuery, metadata, theme, inputClass, mutedClass, hoverClass, namespaceLabel }) {
...
function SQLOutputBlock({ title, value, theme, filenamePrefix }) {
...
function normalizeSQLConsoleConfig(config = {}) {
...
function metadataStatusText(metadata, config) {
...
function recentSQLQueries(items, config) {
  const seen = new Set();
  return [...(items || [])]
...
function tableBrowserSummary(metadata, rows, browserLabel) {

...
- Frontend connector action status, retry classification, and required-field contracts
  are generated from the backend REST contract and untrusted responses are validated at
  the caller boundary.
- Local release checks and CI now share one ratcheted verification policy, command
  context, tooling ownership model, and frontend release gate inventory.

### Fixed

- Retention setting mutations are serialized so stale completions cannot replace newer
  UI state.
- The MCP bridge preserves request identity, Retry-After metadata, timeout
...
- Generated verification recipe digests use a narrow semantic Gitleaks allowlist while
  default secret-detection rules and complete-history scanning remain active.
...
- Independent credential deliveries can now proceed concurrently while credential,
  permission, trust, and token mutations retain an exclusive delivery fence.
...
- Connector request identities now use workspace-bound keyed tags, while browser retries
  use bounded transactional IndexedDB reservations, exact key-and-revision
  reconciliation, and non-extractable origin-local signing keys without persisting raw
  action input.
- MCP connector output authorization is revalidated at the response delivery boundary,
  withholding request and result content when token, Vault, or runtime authorization has
  changed.
- Connector credential provisioning and cleanup now apply an unconditional
  credential-output boundary to responses, failures, and compensation audits, including
  encoded credential forms and dynamically generated S3 authorization headers.
...
- Approved connector actions register their dispatch boundary before external execution,
  distinguish pre-dispatch invalidation from uncertain remote outcomes, and preserve
  pending approvals across transient storage errors.
- Encrypted database runtimes, catalog recovery, and legacy migration now use
  cross-process ownership locks so a second gateway cannot rewrite or recover a database
...
- Database imports and catalog moves now use atomic no-clobber publication and
  whole-artifact fail-closed recovery so concurrent names, stale journals, and rollback
  conflicts cannot overwrite or mix sidecars with a newer database.
- Backup downloads and connector file listings now reject credential reflection across
  streamed bodies and structured provider responses before local publication.
- Database snapshots now honor request cancellation, enforce a gateway-wide concurrency
  and size bound, and remove oversized temporary snapshots before delivery.
- Connector execution now rejects missing or gateway-owned result statuses instead of
  silently recording them as completed, and the MCP package preserves safe
  outcome-unknown request and retry metadata.
- Browser mutation retries now fail closed when durable IndexedDB state is unavailable
  and retain carried idempotency keys across pre-handler authorization or lock
  responses.
...
- Unlocked workspaces now enforce configured history and audit retention every hour
  instead of waiting for the next unlock, with supporting indexes and query shapes for
  high-volume local use.
- Connector integer fields now use exact shared semantics and reject fractional, unsafe,
  or out-of-range values consistently.
- Source builds now require Go 1.26 or newer while the repository remains pinned to the
  reviewed Go 1.26.6 toolchain.

### Fixed

- Redis and Valkey scans and collection previews now surface malformed protocol replies,
  deadline failures, and iteration errors instead of recording false completion.
- Live connector consoles now preserve unexpected runtime lookup, database,
  cancellation, and deadline errors while continuing only when a runtime surface is
  explicitly unavailable.
- Connector background finalization, Mail deadline cleanup, and S3 multipart abort
  failures are now reported instead of being silently discarded.

### Security

- Concurrent first-time gateway-secret creation now uses an exclusive winner-and-reload
  flow with durable file and directory synchronization, preventing competing processes
  from silently selecting different secrets.
- The Go cryptography dependency now includes the SSH channel deadlock denial-of-service
  fixes reported as GO-2026-6354 and GO-2026-6355.
- The MCP package now resolves query-string parsing through the patched dependency
  release that closes GHSA-x5fp-wj9c-mxmx and GHSA-4mjr-xmp4-gh2g.

### Maintenance

- Setup documentation now uses version-neutral placeholders, and the local audit
  guarantee now clearly describes configured retention and deletion boundaries.
- Focused regression coverage now locks retention scheduling, exact integer decoding,
  runtime error propagation, gateway-secret races, and connector cleanup failures.
...
- A client-aware MCP setup command now configures supported local AI clients at user or
  project scope and installs the native AIPermission operator skill where the client
  supports Agent Skills.
- A secret-safe setup doctor now validates client configuration, installed skills,
  gateway reachability, and a real MCP protocol handshake without printing reusable
  credentials.
- Native setup support now covers Codex, Claude Code, Cursor, VS Code, GitHub Copilot
  CLI, Windsurf Cascade, Antigravity IDE and CLI, Gemini CLI, Grok Build, and
  standards-compatible custom agents.

### Changed

- MCP client metadata, configuration formats, scope behavior, documentation, and
  frontend setup guidance now derive from one shared compatibility catalog.
- Operator instructions now use the canonical native skill package instead of legacy
  always-on rule files when a supported client provides Agent Skills.

### Security

- Setup rejects unknown flags, unsafe symbolic-link or junction targets, Git-tracked
  secret-bearing project files, and non-private writes while preserving side-effect-free
  print output.
- Configuration and skill installation use atomic private writes, and diagnostic output
  redacts credential-bearing values and parser failures.

### Maintenance

- The MCP package now locks the supported-client compatibility matrix, setup lifecycle,
  package contents, and real protocol negotiation with focused regression coverage.
- Client setup documentation now distinguishes native skills, legacy rule formats, local
  and hosted client boundaries, project scope, Node.js requirements, and Windows
  execution guidance.

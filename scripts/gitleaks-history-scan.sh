...
if ! shallow_repository="$(docker run --rm \
  --network none \
  -v "$repo_root:/repo:ro" \
  --entrypoint git \
  "$image" \
...
case "$shallow_repository" in
  false) ;;
  true)
    echo "gitleaks history preflight failed: shallow repositories cannot prove full-history coverage" >&2
    exit 1
    ;;
  *)
    echo "gitleaks history preflight failed: mounted repository depth is invalid" >&2
    exit 1
    ;;
esac

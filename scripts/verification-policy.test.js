...
const {
  loadPolicy, validateRequiredCheckMigrations, verifyActionPinsInSource,
  verifyNoRemovals, verifyWorkflows, workflowJobs,
} = require("./verification-policy");

const fixture = path.join(__dirname, "..", "fixture.yml");
const gatePolicy = { required_checks: [
  { name: "Gate", workflow: "fixture.yml", job: "gate", job_name: "Gate", commands: ["node verify.js"] },
] };
...
test("workflow parser scopes commands to their owning job", () => {
  const jobs = workflowJobs("name: CI\njobs:\n  first:\n    steps:\n      - run: alpha\n  second:\n    steps:\n      - run: beta\n");
...
test("workflow verification accepts only active exact commands", (t) => {
  ...
  const rejected = [
    "      # run: node verify.js", "      - name: node verify.js", "      - run: node verify.js || true",
    "      - run: node verify.js\n        if: false", "      - run: node verify.js\n        continue-on-error: true",
    "      - run: node verify.js\n        if: ${{ true }}", "      - run: node verify.js\n        continue-on-error: ${{ false }}",
    "      - run: |\n          cat <<EOF\n          node verify.js\n          EOF",
    "      - run: |\n          if false; then\n            node verify.js\n          fi",
    "      - run: node verify.js\n        shell: python",
    "      - run: |\n          node prepare.js\n          node verify.js",
...
test("workflow verification rejects dynamic check identities", (t) => {
  const write = useFixture(t);
  write(workflow("      - run: node verify.js", "  collision:\n    name: ${{ 'Gate' }}\n    steps:\n      - run: echo collision\n"));
...
test("external workflow actions require immutable pins", () => {
  ...
  for (const reference of ["actions/checkout@v4", "actions/checkout@main", "actions/checkout@deadbeef"])
    assert.throws(() => verify(reference), /full 40-character commit SHA/);
  assert.throws(() => verifyActionPinsInSource("steps:\n  - { uses: actions/checkout@v4 }\n", "fixture.yml"), /full 40-character commit SHA/);
...
test("verification policy ratchet rejects removed gates and tests", () => {
  ...
  for (const key of ["required_checks", "recovery_tests", "connector_conformance_tests", "fuzz_targets"]) {
    const reduced = structuredClone(policy);
    reduced[key].pop();
    assert.throws(() => verifyNoRemovals(policy, reduced), new RegExp(`${key} removed required entry`));
  }
});

test("verification policy permits only an exact declared check migration", () => {
  const shared = { recovery_tests: ["recovery"], connector_conformance_tests: ["conformance"], fuzz_targets: ["fuzz"] };
  const previous = { ...shared, required_checks: [
    { name: "Gate", workflow: "old.yml", job: "matrix", job_name: "Gate (${{ matrix.kind }})", commands: ["node verify.js"] },
  ] };
  const current = { ...shared, required_checks: [
    { name: "Gate", workflow: "new.yml", job: "static", job_name: "Gate", commands: ["node verify.js"] },
  ], required_check_migrations: [{
    name: "Gate", from: { workflow: "old.yml", job: "matrix", job_name: "Gate (${{ matrix.kind }})" },
    to: { workflow: "new.yml", job: "static", job_name: "Gate" }, reason: "Use one stable protected check identity.",
  }] };
  ...
  assert.throws(() => validateRequiredCheckMigrations(redirected), /does not match its current gate/);
